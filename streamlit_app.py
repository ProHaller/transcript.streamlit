    ...
    with tab1:
        ...
        if uploaded_file:
            ...
            if st.button("Transcribe"):
                st.session_state['transcription_text'] = transcription(uploaded_file, language, prompt, response_format)
    ...
    with tab2:
        ...
        if st.button("Process text"):
            completion_text = openai_completion(
                input_text=processing_prompt + (st.session_state['transcription_text'] if st.session_state['transcription_text'] else ""),
                system_prompt="",
                format="text",
                model=model,
                temperature=temperature,
            ...
            if completion_text:
                st.download_button(
                    ...
                    file_name=uploaded_file.name.rsplit('.', 1)[0] + '_processed' + ".txt",
                )
